		...
		<table style="width:100%">
			...
			<c:forEach var="e" items="${list}">
				<tr>
					...
					<td>${e.studentName}</td>
					<td>${e.grade}</td>
					<td><button type="button" class="btn btn-outline-success btn" onclick="location.href='${pageContext.request.contextPath}/employee/deleteStudent?studentNo=${e.studentNo}'">삭제</button></td>
				</tr>
				<tr>
					<td colspan="4"><hr></td>
				</tr>
		...
		<div>
			<div style="text-align: center;">
				...
				<c:if test="${currentPage<lastPage}">
					<a href="${pageContext.request.contextPath}/employee/studentList?currentPage=${currentPage+1}&searchWord=${searchWord}">다음</a><span>&nbsp;</span>
					<a href="${pageContext.request.contextPath}/employee/studentList?currentPage=${lastPage}&searchWord=${searchWord}">끝</a>
				</c:if>
				</h5>
			</div>
			<br><br>
			<form method="get" action="${pageContext.request.contextPath}/employee/studentList">
				<span>
					...
					<span style="float:right"><button type="button" class="btn btn-outline-success btn" onclick="location.href='${pageContext.request.contextPath}/employee/addStudent'">학생등록</button></span>
				</span>
			</form>
		</div>
	</div>
